...
### Overall
```{r overall-rating-plot, fig.align = 'center'}
restaurants%>%
...
```{r high-var-plot, fig.align = 'center'}
restaurants%>%
  group_by(categories)%>%
...
```{r low-variance-categories, fig.align = 'center'}
set.seed(1)
restaurants%>%
  group_by(categories)%>%
  mutate(total = n_distinct(business_id), var = var(stars))%>%
...
```{r individual-reviews-distribution, fig.align='center'}
sample_reviews%>%
  group_by(stars)%>%
  summarise(total = n())%>%
...
```{r individual-reviews-low-var, fig.align='center'}  
...
```{r review-text-by-categories, fig.align='center'}
total_words_by_review%>%
...
```{r average-sentiment-by-rating-table}
...
```{r average-sentiment-service-reviews}
avg_sent <- word_per_review%>%
  inner_join(get_sentiments('afinn'), by = 'word')%>%
  inner_join(sample_reviews, by = 'review_id')%>%
...
reviews over time is simultaneously giving greater emphasis to the words
you'd expect in a 5 star review --amazing, fantastic, etc. So the presence 
of these words in shorter and shorter reviews is pushing up the averages 
over time. 
...
One question you should have when looking at this plot is
...
You see that in the trend line regressing 
...
Another step towards understanding the difference seen in categories is looking at the 
average sentiment of their reviews. 
```{r sentiemtn-score-by-category, fig.align='center'}
word_per_review%>%
  inner_join(sample_reviews, by = 'review_id')%>%
  left_join(restaurants, by = 'business_id')%>%
  inner_join(get_sentiments('afinn'), by = 'word')%>%
...
One last way we will examine review sentiment is to see which
words have the greatest influence on the sentiment of reviews. We do this by 
looking at the number of times that a word occurs in a review and multiplying that
by the words sentiment score. 

As we can see below, the contributions look quite different for each
...
of the English language. I have a feeling if we were to look at bigrams of review text,
we'd see that what actually occurs most often in 1 star reviews are 'didn't love' or
'not recommend' rather than 'love' and 'recommend'. This sort of negation is missed by word level tokenization, but could be
easily captured by bigrams if we felt so inclined.

Still, the visualization is quite informative even with it's flaws. We see more and more positive
influence as we go from 1 to 5 star reviews, which tells us we are capturing the general 
idea of a review even if we are missing some details.
...
related info.  But, one must pick and choose one a dataset like this when confronted 
with the realities of limited time. 

This initial exploration is part one of my Yelp dataset series. Part two is going to build
